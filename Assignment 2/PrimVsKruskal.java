...
public class PrimVsKruskal{
	...
	*/
	static boolean PrimVsKruskal(double[][] G){
		...
		// Scans the array for edges and adds, then checks them one by one
		...
		for (int i = 0; i < n; i++) {
			for (int j = i; j < n; j++) {
				...
				// The following code is for an early detection system
				...
				for (int a = 0; a < n; a++) {
					for (int b = a; b < n; b++) {
						if (prim_matrix[a][b] != kruskal_matrix[a][b]) {
							pvk = false;
							break;
						}
					}
